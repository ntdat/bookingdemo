...
(function($){
    ...
    $.fn.DOPFrontendBookingCalendarPRO = function(options){
         ...
         * Private variables.
         */
        var Data = {"loadURL": "http://booking.com/public/template/js/dopbcp/php-file/load.php",
...
// 2. Schedule
        ...
        methods_schedule = {
             ...
             */
                var scheduleBuffer = {};
                $.post(ajaxLoadURL, {dopbcp_calendar_id: ID,  _token:token, booktype: booktype,
...
// 22. Order
        ...
        methods_order = {
             ...
             */    
               ...
               var hotelid = $('#hotelid').attr('data-id');
               var hotelstar = $('#hotelid').attr('data-star');
               var hotellocation = $('#hotelid').attr('data-location');
               var namehotel = $('#hotelid').text();
                var nameurl = window.location.pathname;
                if(nameurl.search("hotel")){
                    $.post(ajaxSendURL, {dopbcp_calendar_id: ID,
                        ...
                        if(url1.search('/template/Cart') !=-1){

                            location.reload();
                        }
                        else{
                            if(confirm("Added Successful !!!. Are you want check out now ?"))
                            {
                                var baseUrl = window.location.origin;
                                window.location.href= baseUrl+"/template/CheckOutStep1";
                            }
                        }
                        if (data === 'unavailable'){
